...
BEGIN
  ...
  LOOP
  EXIT WHEN C = 0;
    R.NAME := DBMS_RANDOM.STRING('A',4)||ROSE;
    R.INFO := 'INFO'||DBMS_RANDOM.STRING('A',200);
    R.PHOTO := 'https://loclalhost:3000/roses/photos/'||CEIL(DBMS_RANDOM.VALUE(1,1000));
    
    BEGIN
        ADD_ROSE(R.NAME,R.INFO,R.PHOTO,5);
    EXCEPTION WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE(SQLERRM);
    END;
    C := C-1;
...
BEGIN
  ...
  LOOP
  EXIT WHEN C = 0;
    F.ADDRESS := 'ADRESS'||DBMS_RANDOM.STRING('A',20);
    F.PHONE := '+37529'||CEIL(DBMS_RANDOM.VALUE(10000,100000))||CEIL(DBMS_RANDOM.VALUE(10,100));
    
    BEGIN
      ADD_INSPECTION(F.REGION,F.ADDRESS,F.PHONE,5);
    EXCEPTION WHEN OTHERS THEN
      DBMS_OUTPUT.PUT_LINE(SQLERRM);
    END;
    C := C-1;
